...
public interface IFlexibleColorPicker {
    Color GetColor();
...
public class FlexibleColorPicker : MonoBehaviour, IFlexibleColorPicker {
    ...
    */
    ...
    private BufferedColor PickColor1D(BufferedColor color, PickerType type, float value) {
        ...
        return type switch {
            ...
            PickerType.H => color.PickH(value * HUE_LOOP),
    ...
    */
    ...
    private RectTransform GetMarker(Image picker, string search) {
        for (int i = 0; i < picker.transform.childCount; i++) {
            ...
            match &= string.IsNullOrEmpty(search) || candidateName.Contains(search);
    ...
    */
    ...
    private float GetValue1D(PickerType type) {
        ...
        return type switch {
            PickerType.R => Mathf.InverseLerp(min, max, bufferedColor.R),
            PickerType.G => Mathf.InverseLerp(min, max, bufferedColor.G),
            PickerType.B => Mathf.InverseLerp(min, max, bufferedColor.B),
            PickerType.H => Mathf.InverseLerp(min, max, bufferedColor.H / HUE_LOOP),
            PickerType.S => Mathf.InverseLerp(min, max, bufferedColor.S),
            PickerType.V => Mathf.InverseLerp(min, max, bufferedColor.V),
            PickerType.A => Mathf.InverseLerp(min, max, bufferedColor.A),
            _ => throw new Exception("Picker type " + type + " is not associated with a single color value."),
        };
    }
    ...
    */
    ...
    private void TypeHex(string input, bool finish) {
        ...
        if (ColorUtility.TryParseHtmlString(parseText, out Color newColor)) {
            if (bufferedColor != null) {
    ...
    */
    ...
    public static Color ParseHex(string input, Color defaultColor) {
        string parseText = GetSanitizedHex(input, true);
        if (ColorUtility.TryParseHtmlString(parseText, out Color toReturn)) {
            return toReturn;
        }
        else {
            return defaultColor;
